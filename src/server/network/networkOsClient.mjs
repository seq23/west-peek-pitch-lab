...
export function getNetworkOsConfig(env = {}) {
  return {
    ...
    intakeUrl: String(env.NETWORK_OS_PITCH_LAB_ENDPOINT || env.NETWORK_OS_INTAKE_URL || '').trim(),
    sharedSecret: String(env.NETWORK_OS_SHARED_SECRET || '').trim(),
    timeoutMs: Number(env.NETWORK_OS_TIMEOUT_MS || 15000)
  };
}

export async function submitPitchLabShare({ founder, storyCard, consent }, env = {}, fetchImpl = fetch) {
  const config = getNetworkOsConfig(env);
  const payloadResult = buildPitchLabNetworkPayload({ founder, storyCard, consent });
  ...
  try {
    ...
    return { ok: true, intake_id: data.intake_id, review_status: data.review_status, contact_created: false };
  } catch {
